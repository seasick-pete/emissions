...
export const emissionsChart = chartFactory({
  margin: {
      left: 50,
      right: 50,
      top: 50,
...
emissionsChart.loadData = async function loadData(uri) {
  if (uri.match(/.csv$/)) {
...
emissionsChart.init = function initChart(chartType, dataUri, ...args) {
...
emissionsChart.nextSequence = function() {
  this.sequence.push(
    this.sequence.shift()
  );
};

emissionsChart.scatterByYearTypeSize = function ScatterByYearTypeSize(_data) {
...
emissionsChart.update = function update(_data) {
